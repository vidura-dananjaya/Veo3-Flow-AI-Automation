...
- **Last Frame Screenshot Extraction:** Updated `example_client.py` to extract and save the last frame of the generated video as an image when both `videoMode` and `takeLastScreenShot` are enabled in the payload, utilizing OpenCV (`cv2`).
