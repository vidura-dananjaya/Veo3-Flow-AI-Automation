...
- Output this directly in English to `[new skill folder path]/output/publish-details.txt`.
- When generating the description and hashtags, analyze similar niche competitive posts to provide the best possible output.
- Instruct the skill to analyze `[new skill folder path]/examples/publish-details.txt` to understand the exact expected format.
